...
namespace BepuPhysics.CollisionDetection
{
    ...
    {        
        ...
        static void GetNextNormal<T>(ref Simplex simplex, in Vector3Wide support, ref Vector<int> terminatedLanes,
            in Vector3Wide bestNormal, in Vector<Number> bestDepth, in Vector<Number> convergenceThreshold,
            out Vector3Wide nextNormal)
        {
            ...
            Vector3Wide.Scale(bestNormal, Vector.Max(Vector<Number>.Zero, bestDepth), out var searchTarget);
            var terminationEpsilon = Vector.ConditionalSelect(Vector.LessThan(bestDepth, Vector<Number>.Zero), convergenceThreshold - bestDepth, convergenceThreshold);
            ...
            if (typeof(T) == typeof(HasNewSupport))
            {
                ...
                if (Vector.LessThanAny(simplexFull, Vector<int>.Zero))
                {
                    ...
                    var useABD = Vector.BitwiseAnd(Vector.GreaterThanOrEqual(adPlaneTest, Vector<Number>.Zero), Vector.LessThan(bdPlaneTest, Vector<Number>.Zero));
                    var useBCD = Vector.BitwiseAnd(Vector.GreaterThanOrEqual(bdPlaneTest, Vector<Number>.Zero), Vector.LessThan(cdPlaneTest, Vector<Number>.Zero));
                    var useCAD = Vector.BitwiseAnd(Vector.GreaterThanOrEqual(cdPlaneTest, Vector<Number>.Zero), Vector.LessThan(adPlaneTest, Vector<Number>.Zero));
            ...
            var outsideAB = Vector.LessThan(abPlaneTest, Vector<Number>.Zero);
            var outsideBC = Vector.LessThan(bcPlaneTest, Vector<Number>.Zero);
            var outsideCA = Vector.LessThan(caPlaneTest, Vector<Number>.Zero);
            ...
            var simplexDegenerate = Vector.LessThanOrEqual(triangleNormalLengthSquared, longestEdgeLengthSquared * Constants.C1em10);
            var degeneracyEpsilon = new Vector<Number>(Constants.C1em14);
            ...
            Vector3Wide.ConditionallyNegate(Vector.LessThan(calibrationDot, Vector<Number>.Zero), ref triangleNormal);
            ...
            if (Vector.LessThanAny(useEdge, Vector<int>.Zero))
            {
                ...
                var inverseABLengthSquared = Vector<Number>.One / abLengthSquared;
                var inverseBCLengthSquared = Vector<Number>.One / bcLengthSquared;
                var inverseCALengthSquared = Vector<Number>.One / caLengthSquared;
                ...
                var abScaledT = Vector.Max(Vector<Number>.Zero, Vector.Min(abLengthSquared, -oaDotAB));
                var bcScaledT = Vector.Max(Vector<Number>.Zero, Vector.Min(bcLengthSquared, -obDotBC));
                var caScaledT = Vector.Max(Vector<Number>.Zero, Vector.Min(caLengthSquared, -ocDotCA));
                ...
                var bcDegenerate = Vector.Equals(bcLengthSquared, Vector<Number>.Zero);
                var caDegenerate = Vector.Equals(caLengthSquared, Vector<Number>.Zero);
                ...
                if (Vector.LessThanAny(Vector.AndNot(useEdge, terminatedLanes), Vector<int>.Zero))
                {
                    ...
                    var originNearestStart = Vector.Equals(t, Vector<Number>.Zero);
                    var originNearestEnd = Vector.Equals(t, Vector<Number>.One);
            ...
            if (Vector.EqualsAny(terminatedLanes, Vector<int>.Zero))
            {
                ...
                Vector3Wide.Scale(triangleToTarget, new Vector<Number>(Constants.C4), out var pushOffset);
                Vector3Wide.Add(searchTarget, pushOffset, out var pushNormalCandidate);
                Vector3Wide.ConditionalSelect(Vector.BitwiseOr(Vector.LessThanOrEqual(bestDepth, Vector<Number>.Zero), targetContainedInEdgePlanes), triangleToTarget, pushNormalCandidate, out triangleToTarget);
                ...
                Vector3Wide.Scale(triangleToTarget, Vector<Number>.One / Vector.SquareRoot(lengthSquared), out nextNormal);
            }
        ...
        public static void FindMinimumDepth(in TShapeWideA a, in TShapeWideB b, in Vector3Wide localOffsetB, in Matrix3x3Wide localOrientationB, ref TSupportFinderA supportFinderA, ref TSupportFinderB supportFinderB,
            in Vector3Wide initialNormal, in Vector<int> inactiveLanes, in Vector<Number> searchEpsilon, in Vector<Number> minimumDepthThreshold, 
            out Vector<Number> depth, out Vector3Wide refinedNormal, int maximumIterations = 25)
        {
#if DEBUG
            Vector3Wide.LengthSquared(initialNormal, out var initialNormalLengthSquared);
            Debug.Assert(Vector.LessThanAll(Vector.BitwiseOr(inactiveLanes, Vector.LessThan(Vector.Abs(initialNormalLengthSquared - Vector<Number>.One), new Vector<Number>(Constants.C1em6))), Vector<int>.Zero));
#endif
        ...
        public static void FindMinimumDepth(in TShapeWideA a, in TShapeWideB b, in Vector3Wide localOffsetB, in Matrix3x3Wide localOrientationB, ref TSupportFinderA supportFinderA, ref TSupportFinderB supportFinderB,
            ref Simplex simplex, in Vector3Wide initialNormal, in Vector<Number> initialDepth,
            in Vector<int> inactiveLanes, in Vector<Number> convergenceThreshold, in Vector<Number> minimumDepthThreshold, 
            out Vector<Number> refinedDepth, out Vector3Wide refinedNormal, int maximumIterations = 50)
        {
            Vector<Number> depthThreshold = minimumDepthThreshold;
        ...
        public struct SimplexWithWitness
        {
            ...
            public Vector<Number> WeightDenominator;
        }
            ...
            out Vector3Wide nextNormal)
        {
            ...
            var relevantFeatures = Vector<int>.One;
            simplex.A.Weight = Vector.ConditionalSelect(terminatedLanes, simplex.A.Weight, Vector<Number>.One);
            simplex.B.Weight = Vector.ConditionalSelect(terminatedLanes, simplex.B.Weight, Vector<Number>.Zero);
            simplex.C.Weight = Vector.ConditionalSelect(terminatedLanes, simplex.C.Weight, Vector<Number>.Zero);
            simplex.WeightDenominator = Vector.ConditionalSelect(terminatedLanes, simplex.WeightDenominator, Vector<Number>.One);
            ...
            if (Vector.LessThanAny(useEdge, Vector<int>.Zero))
            {
                ...
                terminatedLanes = Vector.BitwiseOr(terminatedLanes, Vector.BitwiseAnd(useEdge, Vector.LessThanOrEqual(bestDistanceSquared, terminationEpsilonSquared)));
                {
                    ...
                    relevantFeatures = Vector.ConditionalSelect(useEdge, Vector.ConditionalSelect(useAB, featureForAB, Vector.ConditionalSelect(useBC, featureForBC, featureForCA)), relevantFeatures);
                    Vector3Wide.ConditionalSelect(useEdge, triangleToTargetCandidate, triangleToTarget, out triangleToTarget);
                    var weightEdgeStart = Vector<Number>.One - t;
                    simplex.A.Weight = Vector.ConditionalSelect(useEdge, Vector.ConditionalSelect(useAB, weightEdgeStart, Vector.ConditionalSelect(useBC, Vector<Number>.Zero, t)), simplex.A.Weight);
                    simplex.B.Weight = Vector.ConditionalSelect(useEdge, Vector.ConditionalSelect(useAB, t, Vector.ConditionalSelect(useBC, weightEdgeStart, Vector<Number>.Zero)), simplex.B.Weight);
                    simplex.C.Weight = Vector.ConditionalSelect(useEdge, Vector.ConditionalSelect(useAB, Vector<Number>.Zero, Vector.ConditionalSelect(useBC, t, weightEdgeStart)), simplex.C.Weight);
        ...
        public static void FindMinimumDepth(in TShapeWideA a, in TShapeWideB b, in Vector3Wide localOffsetB, in Matrix3x3Wide localOrientationB, ref TSupportFinderA supportFinderA, ref TSupportFinderB supportFinderB,
            in Vector3Wide initialNormal, in Vector<int> inactiveLanes, in Vector<Number> searchEpsilon, in Vector<Number> minimumDepthThreshold, 
            out Vector<Number> depth, out Vector3Wide refinedNormal, out Vector3Wide witnessOnA, int maximumIterations = 25)
        {
#if DEBUG
            Vector3Wide.LengthSquared(initialNormal, out var initialNormalLengthSquared);
            Debug.Assert(Vector.LessThanAll(Vector.BitwiseOr(inactiveLanes, Vector.LessThan(Vector.Abs(initialNormalLengthSquared - Vector<Number>.One), new Vector<Number>(Constants.C1em6))), Vector<int>.Zero));
        ...
        public static void FindMinimumDepth(in TShapeWideA a, in TShapeWideB b, in Vector3Wide localOffsetB, in Matrix3x3Wide localOrientationB, ref TSupportFinderA supportFinderA, ref TSupportFinderB supportFinderB,
            ref SimplexWithWitness simplex, in Vector3Wide initialNormal, in Vector<Number> initialDepth,
            in Vector<int> inactiveLanes, in Vector<Number> convergenceThreshold, in Vector<Number> minimumDepthThreshold, 
            out Vector<Number> refinedDepth, out Vector3Wide refinedNormal, out Vector3Wide witnessOnA, int maximumIterations = 50)
        {
            Vector<Number> depthThreshold = minimumDepthThreshold;
            if (supportFinderA.HasMargin)
            {
            ...
            var inverseDenominator = Vector<Number>.One / simplex.WeightDenominator;
