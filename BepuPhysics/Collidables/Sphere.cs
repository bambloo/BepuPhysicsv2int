...
namespace BepuPhysics.Collidables
{
    ...
    public struct Sphere : IConvexShape
    {
        ...
        public readonly bool RayTest(in RigidPose pose, Vector3 origin, Vector3 direction, out Number t, out Vector3 normal)
        {
            ...
            var inverseDLength = Constants.C1 / direction.Length();
            ...
            t = -b - (Number)Math.Sqrt(discriminant);
            if (t < -tOffset)
                t = -tOffset;
            normal = (o + d * t) / Radius;
        ...
        public readonly BodyInertia ComputeInertia(Number mass)
        {
            ...
            inertia.InverseInertiaTensor.XX = inertia.InverseMass / ((Constants.C2 / Constants.C5) * Radius * Radius);
    ...
    public struct SphereWide : IShapeWide<Sphere>
    {
        ...
        {
            Unsafe.Add(ref Unsafe.As<Vector<Number>, Number>(ref Radius), index) = source.Radius;
        }
        ...
        public void GetBounds(ref QuaternionWide orientations, int countInBundle, out Vector<Number> maximumRadius, out Vector<Number> maximumAngularExpansion, out Vector3Wide min, out Vector3Wide max)
        {
            ...
            maximumRadius = new Vector<Number>();
            maximumAngularExpansion = new Vector<Number>();
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public void RayTest(ref RigidPoseWide pose, ref RayWide rayWide, out Vector<int> intersected, out Vector<Number> t, out Vector3Wide normal)
        {
            //Normalize the direction. Sqrts aren't *that* bad, and it both simplifies things and helps avoid numerical problems.
            Vector3Wide.Length(rayWide.Direction, out var inverseDLength);
            inverseDLength = Vector<Number>.One / inverseDLength;
            ...
            var tOffset = Vector.Max(Vector<Number>.Zero, -dot - Radius);
    ...
    public struct SphereSupportFinder : ISupportFinder<Sphere, SphereWide>
    {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public void GetMargin(in SphereWide shape, out Vector<Number> margin)
        {
            margin = shape.Radius;
        }
