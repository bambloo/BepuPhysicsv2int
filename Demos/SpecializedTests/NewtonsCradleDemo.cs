...
namespace Demos.SpecializedTests
{
    ...
    public class NewtonsCradleDemo : Demo
    {
        public unsafe override void Initialize(ContentArchive content, Camera camera)
        {
            camera.Yaw = 0;
            camera.Pitch = 0;
            Simulation = Simulation.Create(BufferPool, new DemoNarrowPhaseCallbacks(new SpringSettings(20, 0), Number.MaxValue, 0), new DemoPoseIntegratorCallbacks(new Vector3(0, -10, 0), 0, 0), new SolveDescription(1, 1));

            const int ballCount = 50;
            Number ballRadius = 0.5f;
            Number ballSpacing = 0.08f;
            Number ballHangHeight = 12f;
            Number barSpacing = Constants.C3;
            ...
            var bar0 = Simulation.Bodies.Add(BodyDescription.CreateKinematic(new Vector3(barShape.HalfWidth, ballHangHeight, barSpacing * -0.5f), barShapeIndex, (Number)Constants.C0));
            var bar1 = Simulation.Bodies.Add(BodyDescription.CreateKinematic(new Vector3(barShape.HalfWidth, ballHangHeight, barSpacing * 0.5f), barShapeIndex, (Number)Constants.C0));
            ...
            for (int i = 0; i < ballCount; ++i)
            {
                ...
                var ball = Simulation.Bodies.Add(BodyDescription.CreateDynamic(ballPosition, ballInertia, new CollidableDescription(ballShapeIndex, 0), (Number)0.0f));
